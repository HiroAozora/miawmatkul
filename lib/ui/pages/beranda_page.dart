...
class _BerandaPageState extends State<BerandaPage> {
  ...
  Future<void> _bersihkanJadwalKedaluwarsa() async {
    ...
    if (pengecualianLama.isEmpty)
      return; // Kalau nggak ada yang kedaluwarsa, abaikan

    for (var exc in pengecualianLama) {
      // Ambil data matkul aslinya dari database
      final jadwalAsli = await isarService.isar.jadwalKuliahs.get(
        exc.jadwalId ?? 0,
      );
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text(
                  'Isi dulu biar MiawMatkul bisa kenalan sama kamu~',
          ...
          actions: [
            ElevatedButton(
              onPressed: () async {
                final namaFinal = nameController.text.trim().isNotEmpty
                    ? nameController.text.trim()
                    : 'Sobat';
                final kelasFinal = kelasController.text.trim().isNotEmpty
                    ? kelasController.text.trim()
                    : '-';
                final kampusFinal = kampusController.text.trim().isNotEmpty
                    ? kampusController.text.trim()
                    : '-';

                await prefs.setString('profil_nama', namaFinal);
                await prefs.setString('profil_kelas', kelasFinal);
                await prefs.setString('profil_kampus', kampusFinal);

                setState(() => _namaPanggilan = namaFinal);
            ...
            child: FutureBuilder<List<JadwalKuliah>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return Center(
                    child: Text('Waduh, ada error: ${snapshot.error}'),
                  );
    ...
    return FutureBuilder<JadwalException?>(
      ...
      builder: (context, snapshot) {
        ...
        if (status == 'libur') {
          ...
          warnaBackground =
              Colors.orange.shade50; // Background agak kekuningan biar ngejreng
        }
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Container(
                      width: 4,
                      height: status == 'offline'
                          ? 50
                          : 70, // Agak panjang kalau online/libur
                      decoration: BoxDecoration(
                        color: warnaAksen,
                        ...
                        children: [
                              ...
                              Expanded(
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    fontWeight: FontWeight.bold,
                                    color: Colors.black87,
                                    decoration: status == 'libur'
                                        ? TextDecoration.lineThrough
                                        : null, // Coret kalau libur
                                  ),
                                ),
                              ),
                          ...
                          Row(
                            children: [
                              ...
                              Expanded(
                                flex:
                                    1, // Berbagi tempat 50:50 dengan Ruangan/Link
                                child: Text(
                                  jadwal.dosen ?? '-',
                                  style: TextStyle(
                                    color: Colors.grey[600],
