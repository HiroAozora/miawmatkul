...
class _ProfilPageState extends State<ProfilPage> {
  ...
  Future<void> _muatDataProfil() async {
    ...
    setState(() {
      _nama = prefs.getString('profil_nama') ?? '-';
      _kelas = prefs.getString('profil_kelas') ?? '-';
      _kampus = prefs.getString('profil_kampus') ?? '-';
    });
  }
      ...
      builder: (context) {
        return Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              mainAxisSize:
                  MainAxisSize.min, // Biar tingginya pas dengan isi form
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildMenuCard(
            ...
            subtitle: 'Hapus seluruh jadwal dari HP ini',
            isDestructive: true,
            onTap: () {
              showDialog(
                context: context,
                builder: (BuildContext context) {
                  return AlertDialog(
                    ...
                    content: const Text(
                      'Yakin mau hapus SEMUA jadwal? Data yang sudah dihapus nggak bisa dikembalikan lagi, lho.',
                    ),
                    actions: [
                      TextButton(
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () async {
                          await isarService.isar.writeTxn(() async {
                            await isarService.isar.jadwalKuliahs.clear();
                          });
                          if (context.mounted) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'Semua jadwal berhasil dibersihkan! 🧹',
                                ),
                              ),
                            );
                          }
